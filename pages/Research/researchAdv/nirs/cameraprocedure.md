...
1. Copy the video files to an iMac 
    ...
    - Take out the micro-SD card from the camera, and insert it into the bigger SD case. This case is in the center drawer of Mac 3. 

    - Insert the SD card into the SD card slot on a iMac, which is on the right-hand side. 

    - You should see a disk icon appear on the desktop. 

    - Open the disk icon and find video recordings in one of the sub-folders. 

    - Copy and paste the videos to the Desktop, then eject the SD card from iMac 

    - Put the micro SD card back into the camera and put the camera back into the lab. Also put the SD card case back the drawer. 
...
2. Upload videos to Google Drive 
    ...
    4. The name of participant folder follows this rule: Child’s First Name_Test Month_Test Date_Test Year. The “Test Month” should be labeled in letters, rather than digits. For example, A participant named George participated in our study on January 10th, 2016. We should create a folder named “George_Jan_10_2016”. 
